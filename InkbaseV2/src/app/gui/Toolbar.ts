...
export const aToolbar = (gameObj: GameObject) =>
  gameObj instanceof Toolbar ? gameObj : null;

type Side = 'top' | 'left' | 'right' | 'bottom';

const isSide = (v: any): v is Side =>
  ['top', 'left', 'right', 'bottom'].indexOf(v) >= 0;
...
function sideToPosition(side: Side) {
  if (side === 'top' || side === 'bottom') {
    const y =
      side === 'top'
        ? Config.gui.padding
        : window.innerHeight - Config.gui.padding;
    return Vec(window.innerWidth / 2, y);
  } else {
    const x =
      side === 'left'
        ? Config.gui.padding
        : window.innerWidth - Config.gui.padding;
