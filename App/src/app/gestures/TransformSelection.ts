...
export function transformSelection(ctx: EventContext): Gesture | void {
  ...
  const makeMoveTransform = (p: Position) => TransformationMatrix.identity().translate(p.x, p.y)
  const makePinchTransform = (a: Position, b: Position) => TransformationMatrix.fromLine(Vec.avg(a, b), b)

  function resetTransform() {
    let transform: TransformationMatrix
    if (firstFinger && secondFinger) {
      transform = makePinchTransform(firstFinger.position, secondFinger.position)
    } else if (firstFinger) {
