...
function line(stroke) {
  ...
  const line2 = Line(Vec.clone(stroke[0]), Vec.clone(stroke[stroke.length - 1]));
  let totalDist = 0;
  for (let i = 1; i < stroke.length - 1; i++) {
    totalDist += Line.distToPoint(line2, stroke[i]);
  }
  const length = Line.len(line2);
  return {
    type: "line",
    line: line2,
    length,
...
function arc(points) {
  ...
  const arc2 = Arc(Vec(centerX, centerY), radius, startAngle, endAngle, clockwise);
  const arcDist = Arc.len(arc2);
  let totalDist = 0;
  for (const p of points) {
    totalDist += Arc.distToPointCircle(arc2, p);
  }
  return {
    type: "arc",
    arc: arc2,
...
function circle(points) {
  ...
  const clockwise = Vec.cross(ab, bc) > 0;
  const circle2 = {center, radius, startAngle, endAngle, clockwise};
  let totalDist = 0;
  for (const p of points) {
    totalDist += Arc.distToPointCircle(circle2, p);
  }
  const circumference = 2 * Math.PI * radius;
  const fitness = totalDist / circumference;
  return {type: "circle", circle: circle2, fitness};
}
export default {
  line,
  arc,
