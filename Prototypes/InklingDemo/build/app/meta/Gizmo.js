import {TAU, lerp} from "../../lib/math.js";
...
import {MetaLabel, MetaStruct} from "./MetaSemantics.js";
const arc = SVG.arcPath(Vec.zero, 10, TAU / 4, Math.PI / 3);
export default class Gizmo extends GameObject {
  constructor(a, b) {
    super();
    this.elm = SVG.add("g", SVG.gizmoElm, {class: "gizmo"});
    this.thick = SVG.add("polyline", this.elm, {class: "thick"});
    this.arrow = SVG.add("polyline", this.elm, {class: "arrow"});
    this.arcs = SVG.add("g", this.elm, {class: "arcs"});
    this.arc1 = SVG.add("path", this.arcs, {d: arc, class: "arc1"});
    this.arc2 = SVG.add("path", this.arcs, {d: arc, class: "arc2"});
    ...
    this.angleInDegrees = constraints.linearRelationship(constraints.variable(this.angleInRadians.value * 180 / Math.PI), 180 / Math.PI, this.angleInRadians, 0).y;
...
export const aGizmo = (gameObj) => gameObj instanceof Gizmo ? gameObj : null;
